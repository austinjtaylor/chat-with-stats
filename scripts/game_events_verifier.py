...
class GameEventsVerifier:
    ...
    def get_player_name(self, player_id: str, team_id: str = None) -> str:
        ...
        LIMIT 1
        """
        result = self.db.execute_query(query, {"player_id": player_id, "team_id": team_id})
        if result:
            row = result[0]
    ...
    def get_game_events(self, game_id: str, team: str = None) -> List[Dict[str, Any]]:
        ...
        """.format("AND team = :team" if team else "")
        ...
        for row in result:
            event = {
                ...
                "pull_y": row["pull_y"],
                "pull_ms": row["pull_ms"],
                "line_players": json.loads(row["line_players"]) if row["line_players"] else []
            }
            events.append(event)
    ...
    def format_event_description(self, event: Dict[str, Any], game_info: Dict[str, Any]) -> str:
        ...
        if event_type == 7:  # Pull inbounds
            ...
            thrower = self.get_player_name(event["thrower_id"])
            return f"Huck throwaway from {thrower}" if event.get("thrower_y", 0) > 50 else f"Throwaway from {thrower}"
        
        elif event_type == 13:  # Throwaway by opposing team
            return f"They scored" if event.get("thrower_y", 0) > 50 else "Throwaway by opposing team"
        
        elif event_type in [1, 2]:  # Start D/O Point
            line_players = event.get("line_players", [])
            player_names = [self.get_player_name(pid) for pid in line_players[:6]]  # Show first 6
    ...
    def display_first_point_ufa_style(self, game_id: str):
        ...
        for i, event in enumerate(away_events):
            ...
            yard_line = self.format_yard_line(event.get("receiver_y") or event.get("thrower_y"))
            ...
            if i == 0 and event["event_type"] in [1, 2]:
                ...
                time_str = f"{event.get('event_time', 0)}s" if event.get("event_time") else ""
                print(f"▼ {current_score}  {line_type}  {time_str}")
                print(f"    {description}")
            else:
                time_info = f"{event.get('event_time', '')}s" if event.get('event_time') else ""
