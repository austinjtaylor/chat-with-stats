...
from possession_calculator import (
...
def get_game_details(db, game_id: Optional[str] = None, date: Optional[str] = None, teams: Optional[str] = None) -> dict[str, Any]:
    ...
    home_possessions = calculate_possessions(db, game_id, game["home_full_team_id"], True)
    away_possessions = calculate_possessions(db, game_id, game["away_full_team_id"], False)
    ...
    if home_stats:
        redzone_data = calculate_redzone_stats_for_team(db, game_id, game["home_full_team_id"], is_home_team=True)
    ...
    if away_stats:
        redzone_data = calculate_redzone_stats_for_team(db, game_id, game["away_full_team_id"], is_home_team=False)
        if redzone_data is not None:
...
def get_individual_leaders(db, game_id: str, game: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_stat_leader(stat_column, stat_name):
        ...
        home_leader = db.execute_query(query, {
            "game_id": game_id,
            "team_id": game["home_full_team_id"]
        })
        
        away_leader = db.execute_query(query, {
            "game_id": game_id,
    ...
    home_pm = db.execute_query(pm_query, {
        "game_id": game_id,
        "team_id": game["home_full_team_id"]
    })
    
    away_pm = db.execute_query(pm_query, {
        "game_id": game_id,
        "team_id": game["away_full_team_id"]
    })
