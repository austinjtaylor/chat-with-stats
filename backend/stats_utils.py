...
def format_results(results: list[dict[str, Any]], max_rows: int = 100) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """
...
def validate_query_safety(query: str) -> tuple[bool, str]:
    ...
    if not (query_upper.startswith("SELECT") or query_upper.startswith("WITH")):
        return False, "Only SELECT queries (including WITH clauses) are allowed for safety"
...
def calculate_percentage(numerator: int, denominator: int, precision: int = 1) -> tuple[float, str]:
    """
