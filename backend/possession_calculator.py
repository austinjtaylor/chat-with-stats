...
from stats_utils import calculate_percentage
from ufa_event_types import (
    is_point_start,
    is_point_end,
    is_turnover,
)
...
def calculate_possessions(db, game_id: str, team_id: str, is_home_team: bool) -> Optional[Dict[str, Any]]:
    """
    ...
    events = db.execute_query(events_query, {"game_id": game_id})
...
def calculate_redzone_stats_for_team(db, game_id: str, team_id: str, is_home_team: bool) -> Optional[Dict[str, Any]]:
    """
...
def calculate_team_percentages(stats: Dict[str, Any], opponent_stats: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    ...
    if stats.get("total_attempts", 0) > 0:
        pct, display = calculate_percentage(stats["total_completions"], stats["total_attempts"])
    ...
    if stats.get("total_hucks_attempted", 0) > 0:
        pct, display = calculate_percentage(stats["total_hucks_completed"], stats["total_hucks_attempted"])
    ...
    if has_valid_possession_data:
        ...
        if stats["o_line_points"] > 0:
            pct, display = calculate_percentage(stats["o_line_scores"], stats["o_line_points"])
        ...
        if stats.get("o_line_possessions", 0) > 0:
            pct, display = calculate_percentage(stats["o_line_scores"], stats["o_line_possessions"])
        ...
        if stats["d_line_points"] > 0:
            pct, display = calculate_percentage(stats["d_line_scores"], stats["d_line_points"])
        ...
        if stats.get("d_line_conversions", 0) > 0:
            pct, display = calculate_percentage(stats["d_line_scores"], stats["d_line_conversions"])
        ...
        if stats.get("total_o_points", 0) > 0:
            pct, display = calculate_percentage(stats["total_o_scores"], stats["total_o_points"])
            stats["hold_percentage"] = pct
            stats["hold_percentage_display"] = display
            stats["o_conversion"] = pct
            stats["o_conversion_display"] = display
        ...
        if stats.get("total_d_points", 0) > 0:
            pct, display = calculate_percentage(stats["total_d_scores"], stats["total_d_points"])
            stats["break_percentage"] = pct
            stats["break_percentage_display"] = display
            stats["d_conversion"] = pct
            stats["d_conversion_display"] = display
...
def calculate_redzone_stats(game_id: str) -> dict:
    ...
    def analyze_team_redzone(team_type):
            ...
            # Track possessions and red zone entry
                ...
                if is_turnover(event_type):
                    ...
                    if event_type == 11:  # BLOCK - blocking team gains possession
                        ...
                        current_possession = 'opponent' if current_possession == team_type else team_type
                ...
                if current_possession == team_type:
                    ...
                    if y_coord:
                        ...
                        elif not current_point['attacking_north'] and 20 <= y_coord <= 40:
                            current_point['reached_redzone'] = True
        ...
        redzone_pct = (redzone_goals / redzone_points * 100) if redzone_points > 0 else 0.0
