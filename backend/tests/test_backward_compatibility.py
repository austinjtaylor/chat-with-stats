...
TEST_CASES = [
    ...
    (
        "How many total yards does Austin Taylor have in 2025?",
        ["Austin Taylor", "yards"],
    ),
    ("What are the top goal scorers all-time?", ["goals"]),
    # Team queries
    (
        "What is Atlanta Hustle's record in 2025?",
        ["Atlanta", "Hustle", "wins", "losses"],
    ),
    ("Show me the standings for 2025", ["standing", "wins", "losses"]),
    # Complex queries
    ("Who has the best plus/minus in 2024?", ["plus", "minus", "2024"]),
    (
        "Which players have the most hucks completed this season?",
        ["hucks", "completed"],
    ),
    # Critical semantic tests - ensure correct interpretation
    (
        "Who are the top goal scorers in the UFA?",
        ["375", "goals"],
    ),  # Matt Smith has 375 goals
    (
        "Who are the top scorers in the UFA?",
        ["goals", "assists"],
    ),  # Should include both
]
...
def test_query(
    query: str, expected_keywords: list[str], delay: float = 2.0
) -> tuple[bool, str]:
